import {
  ...
  import { connection } from '../testing-driver-socket/driver-socket.gateway';
  ...
  export class TestingAdminSocketGateway implements OnGatewayConnection {
    @WebSocketServer()
    io: Namespace;
    ...
    clearArrayOfLocations() {
      locations.length = 0;
      this.io.server.of('/test/driver').emit('clearPath')
    }
  
    @SubscribeMessage('setFrequency')
    setNewFrequency(@MessageBody() newFrequency: number) {
      frequency = +newFrequency;
      this.io.server.of('/test/driver').emit('frequency', { frequency });
    }
    ...
    setNewPath(@MessageBody() pathControl: boolean) {
      ...
      this.io.server.of('/test/driver').emit('pathControl', { pathControl });
    }
  
    @SubscribeMessage('setThreshold')
    setNewThreShold(@MessageBody() newThreShold: number) {
      threshold = +newThreShold;
      this.io.server.of('/test/driver').emit('threshold', { threshold });
    }
